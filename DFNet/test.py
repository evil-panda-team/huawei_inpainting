...
class Tester:
    ...
    def process_batch(self, batch, output):
        ...
        for i in range(result.shape[0]):
            img_name = str(batch['result_path'][i]).split('/')[-1]
            img = cv2.imread('../dataset/test/{}'.format(img_name))
            img[0:512,0:500,:] = result[i][0:512,0:500,:]
            cv2.imwrite(str(batch['result_path'][i]), img)
            cv2.imwrite(str(batch['raw_path'][i]), raw[i])
            cv2.imwrite(str(batch['alpha_path'][i]), alpha[i])
#            Rafa change
#            cv2.imwrite(str(batch['result_path'][i]), cv2.resize(result[i],(500, 600))) 
            
#            Vlad change
            # self.results = np.append(self.results, np.expand_dims(result[i], axis=0), axis=0)
            # self.results_ids.append(str(batch['result_path'][i]))
