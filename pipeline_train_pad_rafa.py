...
#    inverted_mask = np.zeros((600,512), dtype=np.uint8)
#    right_boarder_mask = np.zeros((600,512), dtype=np.uint8)
#    right_boarder_mask[:,500:] = 255
    without_pad = False
    for bbox in v:
...
#        inverted_mask[bbox[0]:bbox[2], bbox[1]:bbox[3]] = 255
        if bbox[3]>=500:
            without_pad = True
    ...
    # mask = cv2.copyMakeBorder(mask,6,0,6,6,cv2.BORDER_CONSTANT,value=255)
    # mask = mask[:600,:512]
    ...
    if without_pad:
        ...
        img[:,500:] = (0,0,0)
    # mask[:, 500:] = 255
    # img = cv2.inpaint(img,right_boarder_mask[:512,:],3,cv2.INPAINT_TELEA)
    # img[:,500:] = cv2.inpaint(img[:,500:],inverted_mask[:512,500:],3,cv2.INPAINT_TELEA)
...
# Gaussian + median + inpainting right boarder 31.25771444857173
# Gaussian + median + inpainting right boarder 31.25771444857173